...
extern IM3Function discover_fn;

void host_log(const char* msg, size_t len);
u32 host_request(const char *url, uint32_t len, uint32_t method);
u32 host_response_get_body_as_doc(uint32_t id);
i32 host_html_parse(const char* html, size_t len);
i32 host_query_selector(size_t docId, const char* query, size_t len);
i32* host_query_selector_all(size_t docId, const char* query, size_t len, uint32_t *pInt);
const char* host_node_text(size_t nodeId, uint32_t *pInt);
i32 host_node_query_selector(size_t docId, const char* query, size_t len);
